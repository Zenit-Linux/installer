...
const entries = [
  ...
  ("w_theme_label", "Motyw", "Theme"),
  ("w_theme_dark", "Ciemny", "Dark"),
  ("w_theme_light", "Jasny", "Light"),
  ...
  ("cli_autoinstall_missing_boot_role",
    "Tryb ręczny + rozruch $1 wymaga manual_$2=... w pliku odpowiedzi. Przerywam.",
    "Manual mode + $1 boot requires manual_$2=... in the answer file. Aborting."),
  ("cli_autoinstall_warn_ignored_manual",
    "Ostrzeżenie: partition_mode=erase -- klucze manual_* w pliku odpowiedzi są ignorowane.",
    "Warning: partition_mode=erase -- manual_* keys in the answer file are ignored."),
  ("cli_autoinstall_warn_ignored_swap",
    "Ostrzeżenie: partition_mode=manual -- swap_mode/swap_size_mib w pliku odpowiedzi są ignorowane " &
    "(użyj manual_swap/manual_swapfile_mib zamiast tego).",
    "Warning: partition_mode=manual -- swap_mode/swap_size_mib in the answer file are ignored " &
    "(use manual_swap/manual_swapfile_mib instead)."),
  ("cli_autoinstall_warn_swap_conflict",
    "Ostrzeżenie: podano jednocześnie manual_swap i manual_swapfile_mib -- manual_swap ma priorytet, " &
    "manual_swapfile_mib jest ignorowane.",
    "Warning: both manual_swap and manual_swapfile_mib were given -- manual_swap takes priority, " &
    "manual_swapfile_mib is ignored."),
  ("cli_autoinstall_warn_luks_passphrase_ignored",
    "Ostrzeżenie: luks_passphrase podane, ale luks=false -- hasło jest ignorowane.",
    "Warning: luks_passphrase was given but luks=false -- the passphrase is ignored."),
  ("cli_autoinstall_unknown_key",
    "Ostrzeżenie: nieznany klucz w pliku odpowiedzi: '$1' (literówka?).",
    "Warning: unknown key in the answer file: '$1' (typo?)."),
]
...
const extraTranslations = [
  ...
  ("w_theme_label", "Motiv", "Thème", "Tema"),
  ("w_theme_dark", "Dunkel", "Sombre", "Oscuro"),
  ("w_theme_light", "Hell", "Clair", "Claro"),
  ...
  ("p_erase_dualboot_warn",
    "Dieses Laufwerk enthält auch ein erkanntes anderes System: $1 -- es wird zusammen mit dem restlichen Laufwerk gelöscht.",
    "Ce disque contient aussi un autre système détecté : $1 -- il sera effacé avec le reste du disque.",
    "Este disco también contiene otro sistema detectado: $1 -- se borrará junto con el resto del disco."),
  ("val_password_min", "Mindestens $1 Zeichen.", "Minimum $1 caractères.", "Mínimo $1 caracteres."),

  # -- tryb tekstowy (--server) -----------------------------------------------
  ("cli_banner", "$1 -- Textinstallation (--server-Modus, ohne GUI)",
    "$1 -- installation en mode texte (mode --server, sans interface graphique)",
    "$1 -- instalación en modo texto (modo --server, sin GUI)"),
  ("cli_lang_prompt", "Systemsprache wählen:", "Choisissez la langue du système :", "Elija el idioma del sistema:"),
  ("cli_kb_prompt", "Tastaturlayout wählen:", "Choisissez la disposition du clavier :", "Elija la distribución del teclado:"),
  ("cli_tz_prompt", "Zeitzone wählen:", "Choisissez le fuseau horaire :", "Elija la zona horaria:"),
  ("cli_net_checking", "Netzwerkverbindung wird geprüft...", "Vérification de la connexion réseau...",
    "Comprobando la conexión de red..."),
  ("cli_aborted", "Installation abgebrochen.", "Installation interrompue.", "Instalación cancelada."),
  ("cli_no_disks", "Keine Laufwerke gefunden (lsblk fehlt oder kein Medium vorhanden). Abbruch.",
    "Aucun disque détecté (lsblk indisponible ou aucun support). Abandon.",
    "No se detectaron discos (lsblk no disponible o sin soporte). Cancelando."),
  ("cli_disk_prompt", "Zieldatenträger wählen:", "Choisissez le disque cible :", "Elija el disco de destino:"),
  ("cli_disk_too_small", "Das gewählte Laufwerk ist zu klein (Minimum $1). Wählen Sie ein anderes.",
    "Le disque choisi est trop petit (minimum $1). Choisissez-en un autre.",
    "El disco elegido es demasiado pequeño (mínimo $1). Elija otro."),
  ("cli_disk_live_confirm", "Dies ist das Live-Medium -- wirklich mit diesem Laufwerk fortfahren?",
    "Ceci est le support live -- vraiment continuer avec ce disque ?",
    "Este es el medio live -- ¿continuar realmente con este disco?"),
  ("cli_boot_prompt", "UEFI-Start? (Nein = BIOS/Legacy)", "Démarrage UEFI ? (Non = BIOS/legacy)",
    "¿Arranque UEFI? (No = BIOS/legacy)"),
  ("cli_mode_prompt",
    "Gesamtes Laufwerk $1 löschen und das einfache Layout verwenden? (Nein = vorhandene Partitionen zuweisen)",
    "Effacer tout le disque $1 et utiliser la disposition simple ? (Non = attribuer des partitions existantes)",
    "¿Borrar todo el disco $1 y usar el diseño simple? (No = asignar particiones existentes)"),
  ("cli_erase_confirm", "Verstanden und fortfahren?", "Compris, voulez-vous continuer ?",
    "¿Entendido y desea continuar?"),
  ("cli_fs_prompt", "Dateisystem:", "Système de fichiers :", "Sistema de archivos:"),
  ("cli_fs_prompt_manual", "Dateisystem für root (und /home, falls formatiert):",
    "Système de fichiers pour root (et /home, si formaté) :",
    "Sistema de archivos para root (y /home, si se formatea):"),
  ("cli_swap_size_prompt", "Swap-Größe in MiB", "Taille du swap en Mio", "Tamaño de swap en MiB"),
  ("cli_swap_prompt", "Auslagerungsspeicher (Swap):", "Espace d'échange (swap) :", "Memoria de intercambio (swap):"),
  ("cli_no_partitions", "Keine Partitionen auf diesem Laufwerk gefunden. Abbruch.",
    "Aucune partition détectée sur ce disque. Abandon.",
    "No se detectaron particiones en este disco. Cancelando."),
  ("cli_root_too_small", "Die gewählte Root-Partition ist zu klein (Minimum $1). Wählen Sie eine andere.",
    "La partition root choisie est trop petite (minimum $1). Choisissez-en une autre.",
    "La partición raíz elegida es demasiado pequeña (mínimo $1). Elija otra."),
  ("cli_home_ask", "Separate /home-Partition verwenden?", "Utiliser une partition /home séparée ?",
    "¿Usar una partición /home separada?"),
  ("cli_swap_ask", "Separate Swap-Partition verwenden?", "Utiliser une partition swap séparée ?",
    "¿Usar una partición swap separada?"),
  ("cli_swapfile_ask", "Stattdessen eine Swap-Datei (2 GiB) im Root verwenden?",
    "Utiliser à la place un fichier swap (2 Gio) dans root ?",
    "¿Usar en su lugar un archivo swap (2 GiB) dentro de root?"),
  ("cli_luks_ask", "Root-Partition verschlüsseln (LUKS)?", "Chiffrer la partition root (LUKS) ?",
    "¿Cifrar la partición raíz (LUKS)?"),
  ("cli_luks_pass_prompt", "Verschlüsselungs-Passphrase: ", "Phrase de chiffrement : ", "Frase de cifrado: "),
  ("cli_luks_pass2_prompt", "Passphrase wiederholen: ", "Répéter la phrase de chiffrement : ",
    "Repita la frase de cifrado: "),
  ("cli_luks_empty", "Das Passwort darf nicht leer sein.", "Le mot de passe ne peut pas être vide.",
    "La contraseña no puede estar vacía."),
  ("cli_luks_too_short", "Die Verschlüsselungs-Passphrase muss mindestens $1 Zeichen lang sein.",
    "La phrase de chiffrement doit comporter au moins $1 caractères.",
    "La frase de cifrado debe tener al menos $1 caracteres."),
  ("cli_fullname_prompt", "Vollständiger Name: ", "Nom complet : ", "Nombre completo: "),
  ("cli_username_prompt", "Benutzername: ", "Nom d'utilisateur : ", "Nombre de usuario: "),
  ("cli_username_invalid",
    "Ungültiger Benutzername (Kleinbuchstaben, Ziffern, _ und -, beginnend mit einem Buchstaben oder _).",
    "Nom d'utilisateur invalide (minuscules, chiffres, _ et -, commençant par une lettre ou _).",
    "Nombre de usuario no válido (minúsculas, dígitos, _ y -, empezando por una letra o _)."),
  ("cli_hostname_prompt", "Rechnername [$1]: ", "Nom de l'ordinateur [$1] : ", "Nombre del equipo [$1]: "),
  ("cli_hostname_invalid",
    "Ungültiger Rechnername (Buchstaben, Ziffern und -, kein - am Anfang/Ende).",
    "Nom d'ordinateur invalide (lettres, chiffres et -, sans - au début/à la fin).",
    "Nombre de equipo no válido (letras, dígitos y -, sin - al inicio/final)."),
  ("cli_password_prompt", "Passwort: ", "Mot de passe : ", "Contraseña: "),
  ("cli_password2_prompt", "Passwort wiederholen: ", "Répéter le mot de passe : ", "Repita la contraseña: "),
  ("cli_password_mismatch", "Die Passwörter stimmen nicht überein, versuchen Sie es erneut.",
    "Les mots de passe ne correspondent pas, réessayez.",
    "Las contraseñas no coinciden, inténtelo de nuevo."),
  ("cli_password_too_short", "Das Passwort muss mindestens $1 Zeichen lang sein.",
    "Le mot de passe doit comporter au moins $1 caractères.",
    "La contraseña debe tener al menos $1 caracteres."),
  ("cli_root_password_ask",
    "Separates Root-Passwort festlegen? (Nein = Root-Konto sperren, sudo verwenden)",
    "Définir un mot de passe root séparé ? (Non = verrouiller le compte root, utiliser sudo)",
    "¿Establecer una contraseña de root separada? (No = bloquear la cuenta root, usar sudo)"),
  ("cli_root_password_prompt", "Root-Passwort: ", "Mot de passe root : ", "Contraseña de root: "),
  ("cli_autologin_ask", "Automatische Anmeldung aktivieren?", "Activer la connexion automatique ?",
    "¿Activar el inicio de sesión automático?"),
  ("cli_summary_title", "Zusammenfassung:", "Résumé :", "Resumen:"),
  ("cli_confirm_install", "Installation starten?", "Démarrer l'installation ?", "¿Iniciar la instalación?"),
  ("cli_install_done", "Installation abgeschlossen! Sie können den Computer jetzt neu starten.",
    "Installation terminée ! Vous pouvez maintenant redémarrer l'ordinateur.",
    "¡Instalación completa! Ahora puede reiniciar el equipo."),
  ("cli_install_failed", "Installation fehlgeschlagen: $1", "L'installation a échoué : $1",
    "La instalación ha fallado: $1"),
  ("cli_step_ok", "OK", "OK", "OK"),
  ("cli_step_error", "FEHLER", "ERREUR", "ERROR"),

  # -- --autoinstall ------------------------------------------------------------
  ("cli_autoinstall_banner", "$1 -- automatische Installation (--autoinstall=$2)",
    "$1 -- installation automatique (--autoinstall=$2)",
    "$1 -- instalación automática (--autoinstall=$2)"),
  ("cli_autoinstall_read_fail", "Antwortdatei kann nicht gelesen werden: $1",
    "Impossible de lire le fichier de réponses : $1",
    "No se puede leer el archivo de respuestas: $1"),
  ("cli_autoinstall_disk_missing",
    "Laufwerk '$1' wurde nicht gefunden (disk=... in der Antwortdatei). Abbruch.",
    "Le disque '$1' est introuvable (disk=... dans le fichier de réponses). Abandon.",
    "No se encontró el disco '$1' (disk=... en el archivo de respuestas). Cancelando."),
  ("cli_autoinstall_manual_root_missing",
    "Der manuelle Modus erfordert manual_root=... in der Antwortdatei. Abbruch.",
    "Le mode manuel nécessite manual_root=... dans le fichier de réponses. Abandon.",
    "El modo manual requiere manual_root=... en el archivo de respuestas. Cancelando."),
  ("cli_autoinstall_missing_creds",
    "Die Antwortdatei muss mindestens username=... und password=... enthalten. Abbruch.",
    "Le fichier de réponses doit contenir au moins username=... et password=.... Abandon.",
    "El archivo de respuestas debe contener al menos username=... y password=.... Cancelando."),
  ("cli_autoinstall_bad_username",
    "Ungültiger Benutzername in der Antwortdatei: $1",
    "Nom d'utilisateur invalide dans le fichier de réponses : $1",
    "Nombre de usuario no válido en el archivo de respuestas: $1"),
  ("cli_autoinstall_bad_hostname",
    "Ungültiger Rechnername in der Antwortdatei: $1",
    "Nom d'ordinateur invalide dans le fichier de réponses : $1",
    "Nombre de equipo no válido en el archivo de respuestas: $1"),
  ("cli_autoinstall_starting",
    "Installation ohne Bestätigung wird gestartet (Autoinstall-Modus)...",
    "Démarrage de l'installation sans confirmation (mode autoinstall)...",
    "Iniciando la instalación sin confirmación (modo autoinstall)..."),
  ("cli_autoinstall_perm_warn",
    "Warnung: Die Antwortdatei ist für andere Systembenutzer lesbar ($1) -- sie kann Klartext-Passwörter " &
    "enthalten. Erwägen Sie `chmod 600`.",
    "Avertissement : le fichier de réponses est lisible par d'autres utilisateurs du système ($1) -- il peut " &
    "contenir des mots de passe en clair. Envisagez `chmod 600`.",
    "Advertencia: el archivo de respuestas es legible por otros usuarios del sistema ($1) -- puede contener " &
    "contraseñas en texto plano. Considere `chmod 600`."),
  ("cli_autoinstall_duplicate",
    "Dieselbe Partition ist in der Antwortdatei mehreren Rollen zugewiesen. Abbruch.",
    "La même partition est assignée à plusieurs rôles dans le fichier de réponses. Abandon.",
    "La misma partición está asignada a más de un rol en el archivo de respuestas. Cancelando."),
  ("cli_autoinstall_too_small",
    "Das gewählte Laufwerk oder die Root-Partition ist zu klein (Minimum $1). Abbruch.",
    "Le disque ou la partition root choisi est trop petit (minimum $1). Abandon.",
    "El disco o la partición raíz elegidos son demasiado pequeños (mínimo $1). Cancelando."),
  ("cli_autoinstall_wrong_disk",
    "Die Partition '$1' (manual_$2=...) liegt nicht auf dem durch disk= angegebenen Laufwerk '$3'. Abbruch.",
    "La partition '$1' (manual_$2=...) n'est pas sur le disque '$3' indiqué par disk=. Abandon.",
    "La partición '$1' (manual_$2=...) no está en el disco '$3' indicado por disk=. Cancelando."),
  ("cli_autoinstall_missing_boot_role",
    "Manueller Modus + $1-Boot erfordert manual_$2=... in der Antwortdatei. Abbruch.",
    "Mode manuel + démarrage $1 nécessite manual_$2=... dans le fichier de réponses. Abandon.",
    "Modo manual + arranque $1 requiere manual_$2=... en el archivo de respuestas. Cancelando."),
  ("cli_autoinstall_warn_ignored_manual",
    "Warnung: partition_mode=erase -- manual_*-Schlüssel in der Antwortdatei werden ignoriert.",
    "Avertissement : partition_mode=erase -- les clés manual_* du fichier de réponses sont ignorées.",
    "Advertencia: partition_mode=erase -- las claves manual_* del archivo de respuestas se ignoran."),
  ("cli_autoinstall_warn_ignored_swap",
    "Warnung: partition_mode=manual -- swap_mode/swap_size_mib in der Antwortdatei werden ignoriert " &
    "(verwenden Sie stattdessen manual_swap/manual_swapfile_mib).",
    "Avertissement : partition_mode=manual -- swap_mode/swap_size_mib du fichier de réponses sont ignorés " &
    "(utilisez plutôt manual_swap/manual_swapfile_mib).",
    "Advertencia: partition_mode=manual -- swap_mode/swap_size_mib del archivo de respuestas se ignoran " &
    "(use manual_swap/manual_swapfile_mib en su lugar)."),
  ("cli_autoinstall_warn_swap_conflict",
    "Warnung: sowohl manual_swap als auch manual_swapfile_mib wurden angegeben -- manual_swap hat Vorrang, " &
    "manual_swapfile_mib wird ignoriert.",
    "Avertissement : manual_swap et manual_swapfile_mib ont été donnés ensemble -- manual_swap est " &
    "prioritaire, manual_swapfile_mib est ignoré.",
    "Advertencia: se indicaron manual_swap y manual_swapfile_mib a la vez -- manual_swap tiene prioridad, " &
    "manual_swapfile_mib se ignora."),
  ("cli_autoinstall_warn_luks_passphrase_ignored",
    "Warnung: luks_passphrase wurde angegeben, aber luks=false -- die Passphrase wird ignoriert.",
    "Avertissement : luks_passphrase a été donné mais luks=false -- la phrase est ignorée.",
    "Advertencia: se indicó luks_passphrase pero luks=false -- la frase se ignora."),
  ("cli_autoinstall_unknown_key",
    "Warnung: unbekannter Schlüssel in der Antwortdatei: '$1' (Tippfehler?).",
    "Avertissement : clé inconnue dans le fichier de réponses : '$1' (faute de frappe ?).",
    "Advertencia: clave desconocida en el archivo de respuestas: '$1' (¿error tipográfico?)."),
]
